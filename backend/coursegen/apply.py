...
def apply_content(job, *, selection=None):
    ...
    publish = bool((job.options or {}).get('publish_immediately'))
    ...
    for entry in entries:
        ...
        if note.get('include') and note.get('blocks'):
            _apply_note(tenant, course, topic, note, status, summary)

        quiz = entry.get('quiz') or {}
        if quiz.get('include') and quiz.get('questions'):
            _apply_quiz(tenant, course, topic, quiz, quiz_status, summary)

    if not (summary['notes_created'] or summary['notes_updated']
            or summary['quizzes_created'] or summary['quizzes_updated']):
        raise ApplyError('Nothing was selected to apply.')
...
def _apply_note(tenant, course, topic, note, status, summary):
    """Create or replace the AI reading note for ``topic``.

    Matching is by ``(topic, content_type='notes')`` so a regenerated note
    updates the existing reading rather than stacking a second one.
    """
    ...
    existing = Content.objects.filter(topic=topic, content_type='notes').order_by('order').first()
...
def _apply_quiz(tenant, course, topic, quiz_data, status, summary):
    ...
    reuse = existing is not None and existing.total_attempts == 0
