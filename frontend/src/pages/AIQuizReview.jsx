  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => navigate('/ai-learning')} className="btn-primary">
          Back to AI Learning
        </button>
      </div>
    )
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            <p className={`text-3xl font-bold ${
              attempt?.percentage >= 80 ? 'text-success-600' :
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    {isCorrect && (
                      <span className="text-success-600">✓ Correct Answer</span>
                    )}
                    {wasWrong && (
                      <span className="text-error-600">✗ Your Answer</span>
                    )}
                    {isUserAnswer && isCorrect && (
                      <span className="text-success-600">✓ Your Answer</span>
                    )}
                  </div>
                </div>
              )
            ...
            <div className="h-2 bg-surface-200 dark:bg-surface-700 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  (attempt?.percentage || 0) >= 80 ? 'bg-success-500' :
